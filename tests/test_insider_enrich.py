...
def test_news_at_lower_window_bound_included(tmp_path):
    # trigger_day - news_lookback_days == start of window; the boundary day
    # itself must be included (window is [start, trigger_day)).
    def fake(url):
        if "/v3/reference/tickers/" in url:
            return {"results": {}}
        return {"results": [{"title": "boundary news", "published_utc": "2025-03-12T09:00:00Z"}]}

    client = _client(fake, tmp_path)
    cfg = InsiderConfig(news_lookback_days=5)
    events = enrich_events([mk_event(trigger_day="2025-03-17")], client, cfg)
    assert events[0].has_recent_news is True
    assert events[0].news_headline == "boundary news"
...
def test_malformed_profile_response_falls_back_to_defaults(tmp_path):
    def fake(url):
        if "/v3/reference/tickers/" in url:
            return ["not", "a", "dict"]
        return {"results": []}
...
def test_malformed_news_response_falls_back_to_defaults(tmp_path):
    def fake(url):
        if "/v3/reference/tickers/" in url:
            return {"results": {}}
...
def test_profile_failure_does_not_blank_successful_news(tmp_path):
    def fake(url):
        if "/v3/reference/tickers/" in url:
            raise urllib.error.HTTPError(url, 500, "Server Error", {}, None)  # type: ignore[arg-type]
        return {"results": [{"title": "ACME wins big contract", "published_utc": "2025-03-15T13:00:00Z"}]}
...
def test_filter_cap_at_boundary_passes_inclusive():
    events = [mk_event(market_cap=300_000_000)]
    cfg = InsiderConfig(max_market_cap=300_000_000)
    assert filter_events(events, cfg) == events
