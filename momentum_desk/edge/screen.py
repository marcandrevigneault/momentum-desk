...
class EventRow:
    ...
    fwd_r: float        # standardized R at the fixed exit
...
class FeatureScreen:
    ...
    ic: float           # Spearman rank correlation with forward R
...
def screen_features(rows: list[EventRow], n_bins: int = 10) -> list[FeatureScreen]:
    ...
    for f in FEATURES:
        pairs = [(r.features[f.name], r.fwd_r) for r in rows if r.features[f.name] is not None]
        if len(pairs) < 10:
            continue
        xs = [p[0] for p in pairs]
        ys = [p[1] for p in pairs]
        deciles = _deciles(pairs, n_bins)
        spread = (deciles[-1].mean_fwd_r - deciles[0].mean_fwd_r) if len(deciles) >= 2 else 0.0
        out.append(FeatureScreen(
            name=f.name, kind=f.kind, desc=f.desc, n=len(pairs),
            ic=round(_spearman(xs, ys), 4), lift_spread=round(spread, 4), deciles=deciles,
        ))
    # rank by absolute information coefficient — strongest signal first
    out.sort(key=lambda s: abs(s.ic), reverse=True)
    return out
